...
################################################################################
HD_names = ["Temoin_TRUQ_DNA3","Temoin-positif-TRU-Q","temoin-positif-TRU-Q","TEMOIN-TRUQ","TEMOIN-POSITIF-TRU-Q","Temoin TRU-Q Horizon","TEMOIN-TRU-Q","Temoin-TRUQ-Q","Temoin_TRUQ_DNA3_EQ","Temoin_HD730","TEMOIN_HD730","temoin_HD730","Temoin-HD730","TEMOIN-HD730","temoin-HD730","HD730"]
#HD_names = ["Temoin_TRUQ_DNA3_EQ"]
HD_names = ["Temoin_Seraseq"]
xls_path = "/media/PGM/results_S5/suivi_temoin_Seraseq.xls"
...
for ionxpress in json_dat.keys():
		...
		#if ctl_name == json_dat[ionxpress]['sample'] :
		if re.search(ctl_name,json_dat[ionxpress]['sample'],re.I):
			...
			barcode = ionxpress
                        ...
                        if barcode:
                                #print "sample : %s barcode : %s" % (sample,barcode)
                        ...
                        absentStyle =xlwt.easyxf('alignment: horiz left; font: name Calibri, bold on, height 220; borders: left thin, top thin, bottom thin, right thin;pattern: pattern solid, fore_colour red')
                        ...
                        #print run_name
                        final_sheet.row(0).write(len(header1),run_name+' '+sample+' '+barcode,headerStyle)
                        ...
                        for i in range(xls_sheet.nrows-2):
                                if i<22:
                                        ...
                                        #print "i:"+str(i)+" line:"+str(variant_line)

                                        va_variant_freq = ""

                                        if (xls_sheet.cell(i+2,5).value).startswith("c."):
                                                va_file.seek(0)
                                                va_reader.next()
                                                for va_line in va_reader:
                                                        ...
                                                        if va_line:
                                                                ...
                                                                if variant2check == va_variant:
                                                                        ...
                                                                        break
                                                if va_variant_freq == "?":
                                                        final_sheet.row(i+2).write(len(variant_line),representsInt(va_variant_freq),absentStyle)
                                                else:
                                                        final_sheet.row(i+2).write(len(variant_line),representsInt(va_variant_freq),generalStyle)
                                        else : #in case of intronic or UTR variants there are no c. / p. so chr,pos,ref,alt are tested
                                                #print "NO c."
                                                variant2check = (xls_sheet.cell(i+2,0).value,int(xls_sheet.cell(i+2,3).value),xls_sheet.cell(i+2,5).value,xls_sheet.cell(i+2,6).value)
                                                #print str(xls_sheet.cell(i+2,0).value)+","+str(int(xls_sheet.cell(i+2,3).value))+","+str(xls_sheet.cell(i+2,5).value)+","+str(xls_sheet.cell(i+2,6).value)
                                                va_file.seek(0)
                                                va_reader.next()
                                                for va_line in va_reader:
                                                        #print va_line
                                                        if va_line:
                                                                va_variant = (va_line[1],int(va_line[14]),va_line[15],va_line[16])
                                                                #print str(va_line[1])+","+str(va_line[14])+","+str(va_line[15])+","+str(va_line[16])
