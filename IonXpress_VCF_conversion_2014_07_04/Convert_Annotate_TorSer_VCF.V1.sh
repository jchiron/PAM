...
error_message() {    # $1 error msg $2 error code
		...
		echo " -R Full path targetRegionsBed file for sample (path begins with /)"
        echo ""
...
while getopts "P:L:S:N:A:B:U:X:C:D:I:T:V:R:O:1:2:3:4:" optionName; do
case "$optionName" in
...
R) targetRegionBed="$OPTARG";;                    ## targetRegionBed file
...
echo "Diag: ${PLUGINCONFIG__VA_DIAG}" # debug line
echo "Safir02: ${PLUGINCONFG__VA_SAFIR02}" # debug line
#################  SAMPLE FOLDER ################
...
if [ "$?" != "0" ]; then
  ...
  exit 92
    ...
    if [[ $targetRegionBed == *"IAD112641_173"* ]];then
    #if [[ "${PLUGINCONFIG__VA_SAFIR02}" == "True" ]];then
    # safir02 formating routine
	exec_time "---> Formating tsv file for Safir02 use"
	tsvLink_safir02=${TSP_URLPATH_PLUGIN_DIR}/${sampleID}/CHR/${sampleName}_NGS_VC.tsv
	...
	echo "Copying files..."
	cp `find ${ANALYSIS_DIR} -name 'Ion*.amplicon.cov.xls' | grep -e ${sampleID}` "${TSP_FILEPATH_PLUGIN_DIR}/safir02/${sampleName}_NGS_AC.tsv"
	cp "${TSP_FILEPATH_PLUGIN_DIR}/${sampleID}/CHR/${sampleName}_NGS_VC.tsv" "${TSP_FILEPATH_PLUGIN_DIR}/safir02/"
	echo "Done."
    else
	#echo "Safir02 not selected"
	#echo "${PLUGINCONFIG__VA_SAFIR02}"
    #fi
    #if [[ "${PLUGINCONFIG__VA_DIAG}" == "True" ]];then
