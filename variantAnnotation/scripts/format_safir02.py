...
###### Reprendre le sample name pour nommer les fichiers de sortie pour Safir 02
    ...
    for row in reader:
        ...
        if (row_content[name2index["GENE_REGION"]] == 'exonic') and (row_content[name2index["TYPE"]] != 'synonymous'):
            row_content[name2index["CHROM"]] = 'chr'+non_decimal.sub('', row_content[name2index["CHROM"]])
            ...
            if(row_content[name2index["TRANSCRIPTS"]] != 'NA'):
                ...
                for transcript_data in transcripts.split(","):
                    ...
                    if(nm_data[0] == ref_nm):
                        ...
                        if(len(nm_data)>=3):
                            ...
                            try:
                                ...
                                if d['transcriptDescriptions']:
                                    ...
                                    row_content[name2index["c."]] = nuc_hgvs
                                    #print 'HGVSc '+row_content[name2index["c."]]
                                else:
                                    succes = False
                                ...
                                if d['proteinDescriptions']:
                                    ...
                                    row_content[name2index["p."]] = prot_hgvs
                                    #print 'HGVSp '+row_content[name2index["p."]]
                                else:
                                    succes = False
                        ...
                        #if(len(nm_data)>=4):
                        if(row_content[name2index["p."]]):
                            m = re.match(r"p\.\((\D+)(?P<codon>\d+).*\)",row_content[name2index["p."]])
                            print m.group(0)
                            #row_content[name2index["p."]] = nm_data[3]
                            #row_content[name2index["Codon"]] = nm_data[3][2:]
                            row_content[name2index["Codon"]] = m.group('codon')
            # Convert [0.00;1.00] ratio into [0;100]
            if(row_content[name2index[ALT_RATIO]] != 'NA'):
                #row_content[name2index[ALT_RATIO]] = format(float(row_content[name2index[ALT_RATIO]]*100),'.1f')
