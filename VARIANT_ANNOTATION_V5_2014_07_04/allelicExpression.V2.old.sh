...
/results/tools/annovar/annotate_variation.pl $hg --filter --dbtype 1000g2012apr_all $unionFile $annoDB &>> $annoLog || exit 10
exec_time "Processing 1000G_EUR..."
/results/tools/annovar/annotate_variation.pl $hg --filter --dbtype 1000g2012apr_eur $unionFile $annoDB &>> $annoLog || exit 11
exec_time "Processing 1000G_AMR..."
/results/tools/annovar/annotate_variation.pl $hg --filter --dbtype 1000g2012apr_amr $unionFile $annoDB &>> $annoLog || exit 12
...
/results/tools/annovar/annotate_variation.pl $hg --filter --dbtype 1000g2012apr_afr $unionFile $annoDB &>> $annoLog || exit 14
for i in ALL EUR AMR ASN AFR; do
	cat $unionFile.hg19_$i.sites.2012_04_dropped | awk '{print $3":"$4":"$5":"$6":"$7"\t"$2}' | sort -k1,1 > 1000G_$i.txt
done
exec_time "Processing COSMIC..."
/results/tools/annovar/annotate_variation.pl $hg --filter --dbtype cosmic67 $unionFile $annoDB &>> $annoLog || exit 15
cat $unionFile.hg19_cosmic67_dropped | awk '{print $3":"$4":"$5":"$6":"$7"\t"$2}' | sort -k1,1 > COSMIC.txt
...
/results/tools/annovar/annotate_variation.pl $hg --filter --dbtype clinvar_20140303 $unionFile $annoDB &>> $annoLog || exit 17
cat $unionFile.hg19_clinvar_20140303_dropped | awk '{print $3":"$4":"$5":"$6":"$7"\t"$2}' | sort -k1,1 > CLINVAR.txt
