...
#T2#/results/tools/annovar/annotate_variation.pl $hg --filter --dbtype 1000g2012apr_all $unionFile $annoDB &>> $annoLog || exit 10
...
#T2#/results/tools/annovar/annotate_variation.pl $hg --filter --dbtype 1000g2012apr_eur $unionFile $annoDB &>> $annoLog || exit 11
...
#T2#/results/tools/annovar/annotate_variation.pl $hg --filter --dbtype 1000g2012apr_amr $unionFile $annoDB &>> $annoLog || exit 12
...
#T# exec_time "Processing 1000G_EAS..."
#T# /results/tools/annovar/annotate_variation.pl $hg --filter --dbtype 1000g2014oct_eas $unionFile $annoDB &>> $annoLog || exit 13
#T# exec_time "Processing 1000G_SAS..."
#T# /results/tools/annovar/annotate_variation.pl $hg --filter --dbtype 1000g2014oct_sas $unionFile $annoDB &>> $annoLog || exit 23
exec_time "Processing 1000G_AFR..."
#T2#/results/tools/annovar/annotate_variation.pl $hg --filter --dbtype 1000g2012apr_afr $unionFile $annoDB &>> $annoLog || exit 14
/results/tools/annovar/annotate_variation.pl $hg --filter --dbtype 1000g2014oct_afr $unionFile $annoDB &>> $annoLog || exit 14
#T2# for i in ALL EUR AMR ASN AFR; do
for i in ALL EUR AMR AFR; do
	#T2# cat $unionFile.hg19_$i.sites.2012_04_dropped | awk '{print $3":"$4":"$5":"$6":"$7"\t"$2}' | sort -k1,1 > 1000G_$i.txt
	cat $unionFile.hg19_$i.sites.2014_10_dropped | awk '{print $3":"$4":"$5":"$6":"$7"\t"$2}' | sort -k1,1 > 1000G_$i.txt
#T# cat $unionFile.hg19_EAS.sites.2014_10_dropped | awk '{print $3":"$4":"$5":"$6":"$7"\t"$2}' | sort -k1,1 > 1000G_ASN.txt
#T# cat $unionFile.hg19_SAS.sites.2014_10_dropped | awk '{print $3":"$4":"$5":"$6":"$7"\t"$2}' | sort -k1,1 >> 1000G_ASN.txt
cat $unionFile.hg19_ASN.sites.2012_04_dropped | awk '{print $3":"$4":"$5":"$6":"$7"\t"$2}' | sort -k1,1 >> 1000G_ASN.txt

done
exec_time "Processing COSMIC..."
#T2#/results/tools/annovar/annotate_variation.pl $hg --filter --dbtype cosmic67 $unionFile $annoDB &>> $annoLog || exit 15
#T2#cat $unionFile.hg19_cosmic67_dropped | awk '{print $3":"$4":"$5":"$6":"$7"\t"$2}' | sort -k1,1 > COSMIC.txt
...
# /results/tools/annovar/annotate_variation.pl $hg --filter --dbtype clinvar_20131105 $unionFile $annoDB &>> $annoLog || exit 17
# cat $unionFile.hg19_clinvar_20131105_dropped | awk '{print $3":"$4":"$5":"$6":"$7"\t"$2}' | sort -k1,1 > CLINVAR.txt
#T2#/results/tools/annovar/annotate_variation.pl $hg --filter --dbtype clinvar_20140303 $unionFile $annoDB &>> $annoLog || exit 17
#T2#cat $unionFile.hg19_clinvar_20140303_dropped | awk '{print $3":"$4":"$5":"$6":"$7"\t"$2}' | sort -k1,1 > CLINVAR.txt
...
#T2#/results/tools/annovar/annotate_variation.pl $hg --filter --otherinfo --dbtype ljb2_all $unionFile $annoDB &>> $annoLog || exit 20 
#T2#cat $unionFile.hg19_ljb2_all_dropped | awk '{print $3":"$4":"$5":"$6":"$7"\t"$2}' | sort -k1,1 > LJB2.txt
...
if [[ $clean == "yes" ]]; then
	#T# rm *_dropped *_filtered tmp.txt tmp2.txt 1000G_ALL.txt 1000G_EUR.txt 1000G_AMR.txt 1000G_EAS.txt 1000G_SAS.txt 1000G_AFR.txt \
