...
class homopolymer(IonPlugin):
	"""Plugin object searching ins/del mutation in homopolymers regions"""
	version = "0.1"
	...
	def launch(self):
		...
		try:
			htmlOut = open('%s/plugin_out/homopolymer_out/homopolymer_block.html'%self.envDict['ANALYSIS_DIR'], 'a+')
		except:
			htmlOut = open('homopolymer_block.html', 'w')
		...
		for vcf in glob.glob('%s/plugin_out/variantCaller_out.%s/IonXpress_*/TSVC_variants.vcf' % (self.envDict['ANALYSIS_DIR'],variantCallerInstance)):
			...
			for row in vcfReader:
				if row[0].startswith('#'):	# pass headers
					continue
				elif "chr20" in row[0] and "31022441" in row[1]: #COSM34210 genomic position
					if len(row[4].split(',')) > 1 : #more than 1 alt allele found
						alleles = row[4].split(',') #array of proposed alleles
						ref_len = len(row[3]) #len of ref allele to found alt difered from 1 base more or less
						for i in range(len(alleles)):
							alt_len = len(alleles[i])
							dist = ref_len-alt_len
							if dist==-1: #dup
								dup_pos = i
							elif dist==1: # del
								del_pos = i
...
#			if not typ:
#				data[sample]["DP"] = 0
#				print "WARNING : no COSM34210 found for %s" % sample
#				htmlOut.write('<br>Warning : see plugin log</b>')
#			else:
...
#						ao_del = ao_del + int(ao[i])
		...
		for sample in sample_list:
			if data[sample]["FREQ_ins"] > threshold:
				data[sample]["DupG"] = "Probable"
			else:
				data[sample]["DupG"] = "Non"
		...
		for sample in sample_list:	
			results.writerow([sample,data[sample]["AO_ins"],data[sample]["AO_del"],data[sample]["DP"],data[sample]["DP_real"],numpy.around(data[sample]["FREQ_ins"],1),numpy.around(data[sample]["FREQ_del"],1),data[sample]["DupG"]])
		csvfile.close()	
		...
		### HTML ###
		reader = csv.reader(open("%s/dupG.csv" % self.envDict['RESULTS_DIR'],"r"), delimiter =";")
		rownum = 0
