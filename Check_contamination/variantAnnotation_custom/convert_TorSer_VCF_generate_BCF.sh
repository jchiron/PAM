...
error_message() {    # $1 error msg $2 error code
	...
	echo "#  into samtools compatible BCF files   #"
        ...
        echo "\t-b\tIonXpress barcode for sample," 
...
TSV_file=${SAMPLE_DIR}/alleles.xls

# 02/09/2015 
#hotspot
hotspotPath=${SAMPLE_DIR}/hotspot.bed
...
#awk -v hotspotPath="$hotspotPath" 'BEGIN {OFS="\t"};$1 !~ "Chrom"{DPF=$23;DPR=$25;SRF=DPF-$28;SRR=DPR-$29;
...
#awk -v hotspotPath="$hotspotPath" 'BEGIN {FS="\t";OFS="\t";};$1 !~ "Chrom"{DPF=$21;DPR=$23;SRF=DPF-$26;SRR=DPR-$27;
#verifify deletion/insertion retreive vcf pos, ref and alt
#status=match($5,/(Heterozygous|Homozygous)/,matchVal);
#if(status){
#	status=match($4,"-",matchVal);#
#	if(status){
#		if($12 != "---"){
#			"grep -m 1 "$12" "hotspotPath" | cut -f 7 | rev | cut -d = -f 1"|getline GETANCHOR;
#			ANCHOR=GETANCHOR;
#			"grep -m 1 "$12" "hotspotPath" | cut -f 2"|getline GETPOS;
#			POS=GETPOS;
#			REF=ANCHOR$3;
#			ALT=ANCHOR;
#		}
...
#			POS=$2-1;
#			if(POS==$15){
#				ANCHOR=substr($16,1,1);
#				REF=ANCHOR$3;
#				ALT=ANCHOR;
#			}
#			else{
#				dist=$2-$15;
#				ANCHOR=substr($16,dist,1);
#				REF=ANCHOR$3;
#				ALT=ANCHOR;
#			}
#		}
...
#		gt="0/1"
#	};
#	print $1,POS,known,REF,ALT,$8,"PASS","AF="$7",SRF="SRF",SRR="SRR",SAF="$26",SAR="$27,"GT:AF:SRF:SRR:SAF:SAR",gt":"$7":"SRF":"SRR":"$26":"$27
#}
#}' ${TSV_file} >> ${outdir}/alleles_${IonXpress}.vcf
...
#	- index BCF
	...
	#bcftools view -bS -D $chromFile - > $workdir/${output}_chr${i}_mpileup.bcf && bcftools index $workdir/${output}_chr${i}_mpileup.bcf
